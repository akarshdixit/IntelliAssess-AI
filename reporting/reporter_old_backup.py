...
Report sections (in order):
  ...
  4. Technical Findings      — per-finding blocks with severity, narrative, remediation
...
class DocxReporter:
    ...
    def _build_technical_findings(self) -> None:
        ...
        Renders each non-INFO AIFindingSummary as a structured subsection:
          - Severity badge + finding title header table
          - Port / Service context
          - Observation (AI narrative or raw_finding_detail fallback)
          - Business Impact (when enriched=True)
          - Recommendation with commands (from AIRemediation when available)
          - Horizontal rule separator

        Findings are ordered as delivered by the enrichment layer
        (already priority-sorted by severity at that stage).
        """
        doc      = self.doc
        ...
        T.add_body_paragraph(
            ...
            "during the assessment. Each finding includes an analyst narrative, business "
            "impact statement, supporting evidence, and remediation guidance."
        )
        T.add_spacer(doc)

        for idx, finding in enumerate(security_findings, start=1):
            self._render_finding_block(idx, finding)
            T.add_spacer(doc, pt=10)

    def _render_finding_block(self, idx: int, finding) -> None:
        """
        Render one AIFindingSummary as a rich structured block.

        Args:
            idx:     Sequential finding number (1-based).
            finding: AIFindingSummary dataclass instance.
        """
        doc      = self.doc
        severity = (getattr(finding, 'severity_label', 'INFO') or 'INFO').upper()
        bg_hex, fg_hex = T.get_severity_colors(severity)
        ...
        T.shade_cell(badge_cell, bg_hex)
        T.set_cell_text(badge_cell, severity, bold=True, color_hex=fg_hex, font_size=10, center=True)
    ...
    def _render_ssl_block(self, parsed) -> None:
        ...
        enabled_protos  = [p["name"] for p in protocols if p.get("enabled")]
        disabled_protos = [p["name"] for p in protocols if not p.get("enabled")]
